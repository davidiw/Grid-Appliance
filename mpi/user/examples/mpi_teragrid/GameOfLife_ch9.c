...
/******************************************
 ...
 in both i and j directions, parallel i/o
...
int main(int argc, char *argv[]){
  ...
  int i, j, n, im, ip, jm, jp, nsum, isum, nprocs, myid;
  ...
  int dim_size[2], periods[2], size[2], subsize[2], start[2];

  float x;

  MPI_Status status;
  MPI_Datatype column_type, mem_type, file_type;
  ...
  dim_size[0] = nprocs/ndims;
  dim_size[1] = nprocs/ndims;

  ninom = NI/dim_size[0];
  njnom = NJ/dim_size[1];

  /* Create virtual topology; presently
     set up for only 1 or 4 procs */

  /* coordinate directions for east-west and north-south */
  ...
  periods[1] = 1;
  reorder = 0;
  MPI_Cart_create(MPI_COMM_WORLD, ndims, dim_size, 
                  periods, reorder, &comm_cart);
  MPI_Cart_shift(comm_cart, ewdir,  1, &mycartid, &proc_east);
  MPI_Cart_shift(comm_cart, ewdir, -1, &mycartid, &proc_west);
  MPI_Cart_shift(comm_cart, nsdir, -1, &mycartid, &proc_north);
  MPI_Cart_shift(comm_cart, nsdir,  1, &mycartid, &proc_south);
  ...
  /* Create derived type for single column of local array.
  ...
  /* write final result to file using parallel i/o */

  if(nprocs == 1){
    fp = fopen("sol9.d","w");
    for(i=1; i<i2; i++){
      fwrite(&new[i][1], sizeof(int), njnom, fp);
    }
    fclose(fp);
  }else{
    ...
    start[1] = 1;
    MPI_Type_create_subarray(2,size,subsize,start,
                             MPI_ORDER_C,MPI_INT,&mem_type);
    MPI_Type_commit(&mem_type);

    /* open file for MPI-2 parallel i/o */

    MPI_File_open(MPI_COMM_WORLD, "sol9.d", MPI_MODE_CREATE|MPI_MODE_WRONLY,
                  MPI_INFO_NULL, &fh);
